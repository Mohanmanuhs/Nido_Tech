      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-800 p-4 rounded-xl flex flex-col items-center">
          ...
          <div className="flex gap-2 mt-4">
            {/* Thumbnail gallery (could be dynamic) */}
            {[product.productImageUrl, product.productImageUrl, product.productImageUrl].map(
              (thumb, i) => (
                <img
                  key={i}
                  src={thumb}
                  alt={`Thumbnail ${i}`}
                  className="w-20 h-20 object-contain border border-gray-700 rounded-lg cursor-pointer hover:border-blue-500"
                />
              )
            )}
          </div>
        ...
        <div className="bg-gray-800 p-6 rounded-xl space-y-6">
          ...
          <div className="pt-4 border-t border-gray-700">
            <p className="text-sm text-gray-400">Delivery by <span className="text-white font-semibold">Tomorrow, 9 AM</span></p>
